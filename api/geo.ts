...
function englishLikely(tags: Record<string, string> | undefined) {
  if (!tags) return false;
  const blob = Object.values(tags).join(' ').toLowerCase();
  if (tags['language:en'] || tags['name:en']) return true;
  if (/\ben\b/.test(tags.languages || '')) return true;
  return /international|english|expat|gaijin|angl/.test(blob);
}
...
async function nominatimSearch(q: string) {
  ...
  const r = await fetch(url, { headers: { 'User-Agent': UA, 'Accept-Language': 'es,en' } });
  if (!r.ok) return null;
...
function parseSites(data: any, lat: number, lng: number) {
  ...
  for (const el of data.elements || []) {
    ...
    sites.push({
      ...
      englishLikely: englishLikely(tags),
    });
  }
...
async function nominatimHealthcare(lat: number, lng: number) {
  const delta = 0.08;
  ...
  for (const q of queries) {
    try {
      ...
      const r = await fetch(url, {
        headers: { 'User-Agent': UA, 'Accept-Language': 'en,es' },
        signal: AbortSignal.timeout(10000),
      });
      if (!r.ok) continue;
      const hits = (await r.json()) as any[];
      for (const hit of hits || []) {
        const name = String(hit.display_name || '').split(',')[0];
        if (!name) continue;
        const key = name.toLowerCase();
        if (seen.has(key)) continue;
        seen.add(key);
        ...
        sites.push({
          ...
          km: Math.round(distanceKm({ lat, lng }, { lat: plat, lng: plng }) * 10) / 10,
          mapsUrl: `https://www.openstreetmap.org/?mlat=${plat}&mlon=${plng}#map=16/${plat}/${plng}`,
          englishLikely: /international|english/.test(blob),
        });
      }
    } catch {
      continue;
...
async function handleLookup(req: Request, res: Response) {
  try {
    ...
    if (english) {
      sites = [...sites].sort((a, b) => Number(b.englishLikely) - Number(a.englishLikely) || a.km - b.km);
    }
